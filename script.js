...
window.StaffDashboard = {
    ...
    billingAction: function(type, itemId, mode) {
        ...
        // Keep active prescription record in longterm_rx array so it remains available for independent printing
        if (type !== 'Rx') {
...
// ========================================================
// 5. CRM AUTOMATED SMS PROTOCOL (RUNS 3RD OF EVERY MONTH)
// ========================================================
window.PromotionalSMS = {
    checkAndTriggerSMS: function() {
        ...
        console.log(`============= CRM SMS DISPATCH LOGS (${now.toLocaleDateString()}) =============`);
        numbers.forEach(phone => {
            const smsText = `Hello ${uniqueContacts[phone]}, Thank you for choosing Namti Drug House! We appreciate your trust in us. We invite you to buy your medicines again this week and enjoy a flat 10% discount on your next tax invoice! T&C apply.`;
            console.log(`[SMS Gateway Broadcast] Sent to: ${phone} | Content: ${smsText}`);
        });

        const bar = document.getElementById('sms-notification-bar');
        if(bar) {
            bar.textContent = `📢 Monthly CRM Promo Campaign Dispatched successfully to ${numbers.length} registered contacts!`;
